...
while(cursor<len(m_src)):
    ...
    if chara == "\"" and not m_src[cursor-1] == "\\":
        ...
        if word in simple_st:
            ...
            _,cursor = m_src,mutth.get_line_end(m_src,cursor)
            if(not mutth.block_line(m_src,cursor)):
                print("please do not place elements on the same line as the deck/object")
                print("sic em' mutt!")
            p_cursor = cursor
            ln_indents = indents
            last_line = None
            while(p_cursor<len(m_src)): 
                if m_src[p_cursor] == "\n":
                    ln_start,_,wrd = mutth.get_next_word(m_src,p_cursor)
                    ln_indents = mutth.get_indent_count(m_src,ln_start)
                    ln_indents_delta = ln_indents
                    if ln_indents <= indents:
                        break
                    if word == "deck":
                        if last_line:
                            m_src = mutth.insert_at(m_src,",",last_line)
                            cursor += 1
                            p_cursor += 1
                        last_line = mutth.get_semicolon(m_src,p_cursor+1)
                p_cursor+=1
            # block semicolon on last element
            if word == "deck" and last_line:
                m_src = mutth.insert_at(m_src," @semicolon_blocked",last_line)
                p_cursor += len(" @semicolon_blocked")
            # add name to end (semicolon and brackets get added later)
            m_src = mutth.insert_at(m_src,"\n"+("\t"*indents)+object_name,p_cursor)
        else:
            m_src,cursor,lind_lvl,ind_delta = mutth.check_indents_add_brackets(m_src,start,lind_lvl)
            finish += ind_delta
            ...
            if (len(word) == 0):
                break
            cursor = mutth.get_semicolon(m_src,cursor)
            # list blocked lines, and enums will not have semicolons
            lw_begin,lw_end,last_word = mutth.get_previous_word(m_src,cursor)
            if last_word == "@semicolon_blocked":
                mod_src = mutth.chop_at(m_src,lw_begin,lw_end+1)
                cursor += len(mod_src)-len(m_src)
                m_src = mod_src
            elif not m_src[cursor-1] == ",":
                m_src = mutth.insert_at(m_src,";",cursor)
    cursor += 1
